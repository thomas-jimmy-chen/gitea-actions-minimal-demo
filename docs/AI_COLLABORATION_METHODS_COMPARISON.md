```
        ▐▛███▜▌
       ▝▜█████▛▘
         ▘▘ ▝▝
    Powered by Claude
```

# AI 協作工作流程方法 - 比較分析

**文檔版本**: 1.0
**建立日期**: 2025-12-31

---

## 文檔索引

本文檔因篇幅較長，已分為兩個部分：

### [第 1 部分](./AI_COLLABORATION_METHODS_COMPARISON-1.md)

| 章節 | 內容 |
|------|------|
| 1. 四種方法總覽 | 方法名稱與一句話描述 |
| 2. 象限圖：需求明確度 × 專案規模 | 依需求/規模選擇方法 |
| 3. 象限圖：變動頻率 × 團隊規模 | 依變動/團隊選擇方法 |
| 4. 特性比較矩陣 | 各方法特性量化比較 |
| 5. 時間軸視角 | 專案生命週期中的方法使用 |
| 6. 決策流程圖 | 快速選擇決策樹 |
| 7. 適用情境對照表 | 10+ 種情境推薦 |
| 8. 文檔產出對照 | 各方法的文檔輸出 |
| 9. 方法轉換路徑圖 | 方法間的轉換路徑 |
| 10. 快速選擇指南 | 一問一答快速定位 |

### [第 2 部分](./AI_COLLABORATION_METHODS_COMPARISON-2.md)

| 章節 | 內容 |
|------|------|
| 11. 流程比較圖 | 各方法的開發流程視覺化 |
| 12. 情境對照與建議 | 依專案類型/團隊狀況建議 |
| 13. 提示詞模板 | 各方法的開始提示詞 |
| 14. AI 工具規則文件 | 各 AI 工具的配置文件 |
| 15. 相關文檔 | 各方法詳細文檔連結 |
| 16. 實際案例：EEBot | 方法 4 的實際實作案例 |

---

## 快速導航

### 按需求選擇

- **需求明確** → [方法 1](./AI_COLLABORATION_METHOD_1_STRUCTURED.md)
- **需求不明** → [方法 2](./AI_COLLABORATION_METHOD_2_ITERATIVE.md)
- **準備過渡** → [方法 3](./AI_COLLABORATION_METHOD_3_TRANSITION.md)
- **部分明確** → [方法 4](./AI_COLLABORATION_METHOD_4_HYBRID.md)

### 按情境選擇

| 我想要... | 建議 |
|-----------|------|
| 快速開始一個小專案 | [第 1 部分 §10](./AI_COLLABORATION_METHODS_COMPARISON-1.md) |
| 了解各方法的差異 | [第 1 部分 §4](./AI_COLLABORATION_METHODS_COMPARISON-1.md) |
| 看決策流程圖 | [第 1 部分 §6](./AI_COLLABORATION_METHODS_COMPARISON-1.md) |
| 取得提示詞模板 | [第 2 部分 §13](./AI_COLLABORATION_METHODS_COMPARISON-2.md) |
| 設定 AI 工具規則 | [第 2 部分 §14](./AI_COLLABORATION_METHODS_COMPARISON-2.md) |
| 看實際案例 | [第 2 部分 §16](./AI_COLLABORATION_METHODS_COMPARISON-2.md) |

---

**文檔建立者**: Claude Code (Opus 4.5)
**用途**: AI 協作工作流程方法的比較與選擇指南
